 ...
 * Modified : Upgraded to Bootstrap 5 and PHP 8.0+ Compatibility
 * Version : 6.4.2 (Modded)
...
$_TRANSLATIONS["pt_BR"] = array(
	"file_name" => "Nome do arquivo", "size" => "Tamanho", "last_changed" => "Modificado em", "total_used_space" => "Total de espaço utilizado", "free_space" => "Espaço livre", "password" => "Senha", "upload" => "Enviar", "failed_upload" => "Falha ao enviar o arquivo!", "failed_move" => "Falha ao mover o arquivo para o diretório correto!", "wrong_password" => "Senha errada", "make_directory" => "Nova pasta", "new_dir_failed" => "Falha ao criar diretório", "chmod_dir_failed" => "Falha ao mudar os privilégios do diretório", "unable_to_read_dir" => "Não foi possível ler o diretório", "location" => "Localização", "root" => "Raíz", "log_file_permission_error" => "O script não tem permissão para escrever o arquivo de log.", "upload_not_allowed" => "A configuração do script não permite envios neste diretório.", "upload_dir_not_writable" => "Não há permissão para escrita neste diretório.", "mobile_version" => "Versão Móvel", "standard_version" => "Versão Padrão", "page_load_time" => "Página carregada em %.2f ms", "wrong_pass" => "Nome de usuário ou senha errados", "username" => "Nome de Usuário", "log_in" => "Log in", "upload_type_not_allowed" => "Não é permitido envio de arquivos deste tipo.", "del" => "Deletar", "log_out" => "Log out"
);
...
$_IMAGES["txt"] = "";
...
class EncodeExplorer
{
	...
	function makeArrow($sort_by)
	{
		...
		return "<a class=\"text-decoration-none text-dark\" href=\"".$this->makeLink(false, false, $sort_by, $sort_as, null, $this->location->getDir(false, true, false, 0))."\">{$text} {$img}</a>";
	}
...
<head>
	...
	<style>
		...
		#thumb { position: absolute; display: none; border: 1px solid #ddd; background: #fff; padding: 4px; border-radius: 4px; z-index: 1050; box-shadow: 0 4px 8px rgba(0,0,0,0.1); }
	</style>
	<?php
...
<div class="container my-4">
	...
	<div class="text-center mb-4">
		<h2 class="text-primary fw-bold">
			<a href="<?php print $this->makeLink(false, false, null, null, null, ""); ?>" class="text-decoration-none">
				<?php if(EncodeExplorer::getConfig('main_title') != null) print EncodeExplorer::getConfig('main_title'); ?>
			</a>
		</h2>
		<?php
		if(EncodeExplorer::getConfig("secondary_titles") != null && is_array(EncodeExplorer::getConfig("secondary_titles")) && count(EncodeExplorer::getConfig("secondary_titles")) > 0 && $this->mobile == false)
		{
			...
			print "<p class=\"text-muted\">".$secondary_titles[array_rand($secondary_titles)]."</p>\n";
		}
		?>
	</div>
	<?php endif; ?>
	...
	<nav aria-label="breadcrumb" class="bg-white p-3 rounded shadow-sm mb-3">
		<ol class="breadcrumb mb-0">
			<li class="breadcrumb-item"><a href="?dir="><i class="bi bi-house-door-fill"></i> <?php print $this->getString("root"); ?></a></li>
			<?php
			for($i = 0; $i < count($this->location->path); $i++) {
				...
				print '<li class="breadcrumb-item"><a href="'.$link.'">'.$this->location->getPathLink($i, true).'</a></li>';
			}
			?>
		</ol>
	</nav>
				...
				<tbody>
					<tr>
						...
						<td colspan="<?php print ($this->mobile == true ? 1 : (GateKeeper::isDeleteAllowed() ? 4 : 3)); ?>">
							<a href="<?php print $this->makeLink(false, false, null, null, null, $this->location->getDir(false, true, false, 1)); ?>" class="text-decoration-none fw-bold">..</a>
						</td>
					</tr>
					<?php
					if($this->dirs) {
						foreach ($this->dirs as $dir) {
							...
							print "<td><a href=\"".$this->makeLink(false, false, null, null, null, $this->location->getDir(false, true, false, 0).$dir->getNameEncoded())."\" class=\"text-decoration-none text-dark fw-bold\">".$dir->getNameHtml()."</a></td>";
							if($this->mobile != true) {
					...
					if($this->files) {
						foreach ($this->files as $file) {
							...
							print "<td><a href=\"$fileLink\" $target class=\"text-decoration-none text-dark file $thumbClass\" $download>".$file->getNameHtml();
	...
	<?php if(GateKeeper::isAccessAllowed() && GateKeeper::showLoginBox()): ?>
	<form enctype="multipart/form-data" method="post" class="mb-4">
		<div class="row align-items-center bg-white p-3 rounded shadow-sm">
			<div class="col-auto">
				<label for="user_name" class="col-form-label"><?php print $this->getString("username"); ?>:</label>
			</div>
			<div class="col-auto">
				<input type="text" name="user_name" id="user_name" class="form-control" />
			</div>
			<div class="col-auto">
				<label for="user_pass" class="col-form-label"><?php print $this->getString("password"); ?>:</label>
			</div>
			<div class="col-auto">
	...
	<div class="text-center text-muted small pb-4">
		<?php
		if(GateKeeper::isUserLoggedIn()) print "<a href=\"".$this->makeLink(false, true, null, null, null, "")."\" class=\"text-decoration-none\">".$this->getString("log_out")."</a> | ";

		if(EncodeExplorer::getConfig("mobile_enabled") == true)
		{
			print "<a href=\"".$this->makeLink(true, false, null, null, null, $this->location->getDir(false, true, false, 0))."\" class=\"text-decoration-none\">\n";
